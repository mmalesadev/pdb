...
AudioTrack::AudioTrack(const std::string & filePath, const float masterVolume) : filePath_(filePath), masterVolume_(masterVolume)
{
    ...
    BOOST_LOG_TRIVIAL(info) << "Audio track created: " << trackName_ << ", masterVolume: " << masterVolume_;
}
